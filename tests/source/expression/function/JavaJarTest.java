...
public class JavaJarTest {
    ...
    private static final String MKDIR_CLASSES_OUT_COMMAND = String.format(JavaJar.MKDIR_COMMAND, CLASSES_OUT);
    private static final String MKDIR_MANIFEST_OUT_COMMAND = String.format(JavaJar.MKDIR_COMMAND, MANIFEST_OUT);
    private static final String MKDIR_JAR_OUT_COMMAND = String.format(JavaJar.MKDIR_COMMAND, JARS_OUT);
    ...
    private static final String JAR_RES_COMMAND = String.format("jar cfm %s %s %s -C %s .", JAR_OUT, MANIFEST_MF, RESOURCE, CLASSES_OUT);
    ...
    public void javaJarResource() {
        ...
        Mockito.verify(match, Mockito.times(1)).runCommand(Mockito.eq(ECHO_COMMAND));
        Mockito.verify(match, Mockito.times(1)).runCommand(Mockito.eq(JAVAC_COMMAND));
        Mockito.verify(match, Mockito.times(1)).runCommand(Mockito.eq(JAR_RES_COMMAND));
    }
